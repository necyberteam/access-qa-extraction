...
| — | allocations | allocations:72170 | *(not an issue)* | Suspected hallucination: "Dan Jurafsky, a renowned professor at Stanford." Verified against source_data — Jurafsky is named explicitly in the Team section of the abstract (~600-word abstract, team section at the bottom). "Renowned" is a fair LLM summary of his listed credentials (MacArthur Fellow, American Academy member, co-author of the standard NLP textbook). Extraction is reading long dense abstracts correctly. | n/a | Positive finding: LLM correctly surfaces team members buried in long abstract text. Judge approved. |
